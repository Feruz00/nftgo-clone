import { GithubFilled, InstagramFilled, YoutubeFilled } from '@ant-design/icons'
import Twitter from '@mui/icons-material/Twitter'
// import TelegramIcon from '@mui/icons-material/Telegram';
import React from 'react'
import { Theme } from '../../theme'
import './Footer.css'
import Telegram from '@mui/icons-material/Telegram';

const Footer = () => {
    
    const {darkMode} = Theme()
  return (
    <div className='nft__footer-container'>
        <div className='nft__footer'>
            <div className='nft__footer-logo'>
                {!darkMode ? <svg width="156" height="38" viewBox="0 0 156 38" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M30.364 38H7.636C3.434 38 0 34.562 0 30.364V7.636C0 3.434 3.434 0 7.636 0h22.728C34.562 0 38 3.434 38 7.636v22.728C38 34.562 34.562 38 30.364 38z" fill="#5A66F9"></path><path d="M23.472 12.528c-.265.091-.165.404.112.4l1.437-.031-2.873 7.705-2.713-4.988a5.277 5.277 0 00-4.285-2.774h-.807c-2.123.161-3.898 1.502-4.623 3.504L5.292 28.502c-.03.083.03.17.117.17h6.629c.052 0 .1-.035.117-.083l2.783-7.657 2.7 4.966a5.28 5.28 0 005.065 2.778 5.295 5.295 0 004.636-3.46l4.615-12.376 1.21-3.377a.126.126 0 00-.16-.16l-9.532 3.225z" fill="#fff"></path><path opacity="0.3" d="M9.715 16.34L5.292 28.501c-.03.083.03.17.117.17h6.628c.052 0 .1-.035.117-.083l2.783-7.657 2.7 4.966c.886 1.628 2.479 2.648 4.28 2.778h.79L14.342 12.84a5.28 5.28 0 00-4.627 3.5z" fill="#3B53FF"></path><path d="M11.807 20.281L8.73 28.507c-.03.082.03.17.117.17h3.195c.052 0 .1-.035.117-.083l2.783-7.658-.56-.872c-.578-.907-2.175-.781-2.575.217z" fill="#fff"></path><path opacity="0.3" d="M25.285 21.27l3.07-8.03a.253.253 0 00-.235-.343h-3.1l-2.873 7.722.56.873c.582.903 2.18.777 2.578-.222z" fill="#3B53FF"></path><path d="M62.507 22.562L51.485 11.907c-.863-.836-2.222-1.126-3.453-.735C46.8 11.562 46 12.529 46 13.639v13.357h3.928V15.438l11.021 10.654c.61.589 1.465.908 2.345.908.369 0 .745-.055 1.11-.172 1.23-.391 2.03-1.358 2.03-2.468V11.18h-3.927v11.382zM69.126 15.19v11.474h3.928V15.189c0-.47.474-.87 1.033-.87h13.222v-3.307H74.088c-2.734 0-4.96 1.874-4.96 4.178z" fill="#151D51"></path><path d="M87.31 17.333H74.587v3.308H87.31v-3.308zM90.64 14.32h7.81v12.348h3.928V14.32h7.809v-3.308H90.641v3.307zM116.337 20.582v-3.261c0-1.652 1.596-2.997 3.555-2.997h11.585v-3.308h-11.585c-4.126 0-7.483 2.829-7.483 6.305v3.261c0 3.476 3.357 6.305 7.483 6.305h7.213v-3.308h-7.213c-1.959-.004-3.555-1.345-3.555-2.997z" fill="#151D51"></path><path d="M127.604 17.295h-7.306v3.308h7.306c.554 0 1.02.294 1.02.643v5.422h3.928v-5.422c0-2.181-2.218-3.95-4.948-3.95z" fill="#5A66F9"></path><path d="M147.966 11.155h-4.875c-4.432 0-8.034 3.039-8.034 6.77v2.19c0 3.733 3.602 6.772 8.034 6.772h4.875c4.432 0 8.034-3.04 8.034-6.771v-2.19c0-3.737-3.602-6.771-8.034-6.771zm4.11 8.96c0 1.909-1.841 3.464-4.105 3.464h-4.876c-2.265 0-4.106-1.551-4.106-3.463v-2.19c0-1.908 1.841-3.463 4.106-3.463h4.876c2.264 0 4.105 1.55 4.105 3.463v2.19z" fill="#151D51"></path></svg>
                    :
                    <svg width="180" height="44" viewBox="0 0 156 38" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M30.364 38H7.636C3.434 38 0 34.562 0 30.364V7.636C0 3.434 3.434 0 7.636 0h22.728C34.562 0 38 3.434 38 7.636v22.728C38 34.562 34.562 38 30.364 38z" fill="#5A66F9"></path><path d="M23.472 12.528c-.265.091-.165.404.113.4l1.437-.031-2.874 7.705-2.713-4.988a5.277 5.277 0 00-4.284-2.774h-.808c-2.123.161-3.898 1.502-4.623 3.504L5.292 28.502c-.03.083.03.17.118.17h6.628c.052 0 .1-.035.117-.083l2.783-7.657 2.7 4.966a5.28 5.28 0 005.066 2.778 5.295 5.295 0 004.636-3.46l4.614-12.376 1.211-3.377a.126.126 0 00-.16-.16l-9.533 3.225z" fill="#fff"></path><path opacity="0.3" d="M9.715 16.34L5.292 28.501c-.03.083.03.17.117.17h6.629c.052 0 .1-.035.117-.083l2.782-7.657 2.7 4.966c.886 1.628 2.479 2.648 4.28 2.778h.79L14.343 12.84a5.28 5.28 0 00-4.628 3.5z" fill="#5A66F9"></path><path d="M11.808 20.281L8.73 28.507c-.03.082.03.17.117.17h3.195c.052 0 .1-.035.117-.083l2.783-7.658-.56-.872c-.577-.907-2.175-.781-2.574.217z" fill="#fff"></path><path opacity="0.3" d="M25.285 21.27l3.07-8.03a.253.253 0 00-.235-.343h-3.1l-2.873 7.722.56.873c.582.903 2.18.777 2.578-.222z" fill="#5A66F9"></path><path d="M62.507 22.562L51.485 11.907c-.863-.836-2.222-1.126-3.453-.735C46.8 11.562 46 12.529 46 13.639v13.357h3.928V15.438l11.021 10.654c.61.589 1.465.908 2.345.908.369 0 .745-.055 1.11-.172 1.23-.391 2.03-1.358 2.03-2.468V11.18h-3.927v11.382zM69.126 15.19v11.474h3.928V15.189c0-.47.474-.87 1.033-.87h13.222v-3.307H74.088c-2.734 0-4.96 1.874-4.96 4.178z" fill="#FCFCFF"></path><path d="M87.31 17.333H74.587v3.308H87.31v-3.308zM90.64 14.32h7.81v12.348h3.928V14.32h7.809v-3.308H90.641v3.307zM116.337 20.582v-3.261c0-1.652 1.596-2.997 3.555-2.997h11.585v-3.308h-11.585c-4.126 0-7.483 2.829-7.483 6.305v3.261c0 3.476 3.357 6.305 7.483 6.305h7.213v-3.308h-7.213c-1.959-.004-3.555-1.345-3.555-2.997z" fill="#FCFCFF"></path><path d="M127.604 17.295h-7.306v3.308h7.306c.554 0 1.02.294 1.02.643v5.422h3.928v-5.422c0-2.181-2.218-3.95-4.948-3.95z" fill="#5A66F9"></path><path d="M147.966 11.155h-4.875c-4.432 0-8.034 3.039-8.034 6.77v2.19c0 3.733 3.602 6.772 8.034 6.772h4.875c4.432 0 8.034-3.04 8.034-6.771v-2.19c0-3.737-3.602-6.771-8.034-6.771zm4.11 8.96c0 1.909-1.841 3.464-4.105 3.464h-4.876c-2.265 0-4.106-1.551-4.106-3.463v-2.19c0-1.908 1.841-3.463 4.106-3.463h4.876c2.264 0 4.105 1.55 4.105 3.463v2.19z" fill="#FCFCFF"></path></svg>
                }
            </div>
            <div className='nft__footer-links'>
                <div className='nft__footer-links-left '>
                    <div>
                        <h2>Join Our Community</h2>
                        <div className='nft__footer-left-icons'>
                            <Twitter />
                            <YoutubeFilled />
                            <InstagramFilled />
                            <Telegram />
                            <GithubFilled />
                        </div>
                    </div>

                    <div>
                        <div className='nft__footer-links-line'></div>
                        <div className='nft__footer-links-copy'>© 2022 NFTGo — All rights reserved</div>
                    </div>
                    
                </div>
                <div className='nft__footer-links-right'>
                    <div>
                        <h3>Sitemap</h3>
                        <a href="/">Analytics</a>
                        <a href="/">Whales</a>
                        <a href="/">Discord Bot</a>
                        <a href="/">Research</a>
                        <a href="/">Drops</a>
                    </div>
                    <div>
                        <h3>Links</h3>
                        <a href="/">Opensea</a>
                        <a href="/">Rarible</a>
                        <a href="/">Super Rare</a>
                    </div>
                    
                    <div>
                        <h3>Support</h3>
                        <a href="/">FAQ</a>
                        <a href="/">Docs</a>
                        <a href="/">Metrics</a>
                        <a href="/">Listing Request</a>
                        <a href="/">Media Kit</a>
                    </div>
                    <div>
                        <h3>Company</h3>
                        <a href="/">About us</a>
                        <a href="/">Terms of Use</a>
                        <a href="/">Privacy Policy</a>
                        <a href="/">Contact us</a>
                        <a href='/' className='nft__footer-hire'>🔥 We're hiring!</a>
                    </div>
                    
                    
                </div>
            </div>
            
        </div>

    </div>
  )
}

export default Footer